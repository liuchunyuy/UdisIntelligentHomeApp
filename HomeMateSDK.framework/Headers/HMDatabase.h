...
#import "HMDatabasePool.h"


#if ! __has_feature(objc_arc)
    #define HMDBAutorelease(__v) ([__v autorelease]);
    #define HMDBReturnAutoreleased HMDBAutorelease

    #define HMDBRetain(__v) ([__v retain]);
    #define HMDBReturnRetained HMDBRetain

    #define HMDBRelease(__v) ([__v release]);

	#define HMDBDispatchQueueRelease(__v) (dispatch_release(__v));
#else
    // -fobjc-arc
    #define HMDBAutorelease(__v)
    #define HMDBReturnAutoreleased(__v) (__v)

    #define HMDBRetain(__v)
    #define HMDBReturnRetained(__v) (__v)

    #define HMDBRelease(__v)

	#if TARGET_OS_IPHONE
		// Compiling for iOS
		#if __IPHONE_OS_VERSION_MIN_REQUIRED >= 60000
			// iOS 6.0 or later
			#define HMDBDispatchQueueRelease(__v)
		#else
			// iOS 5.X or earlier
			#define HMDBDispatchQueueRelease(__v) (dispatch_release(__v));
		#endif
...
#define HMDatabaseSQLiteBusyMicrosecondsTimeout 20

/** A SQLite ([http://sqlite.org/](http://sqlite.org/)) Objective-C wrapper.
 
 ### Usage
 The three main classes in HMDB are:

 - `HMDatabase` - Represents a single SQLite database.  Used for executing SQL statements.
 - `<HMResultSet>` - Represents the results of executing a query on an `HMDatabase`.
 - `<HMDatabaseQueue>` - If you want to perform queries and updates on multiple threads, you'll want to use this class.

 ### See also
 
 - `<HMDatabasePool>` - A pool of `HMDatabase` objects.
 - `<HMStatement>` - A wrapper for `sqlite_stmt`.
 
 ### External links
 
 - [HMDB on GitHub](https://github.com/ccgus/HMDB) including introductory documentation
 - [SQLite web site](http://sqlite.org/)
 - [HMDB mailing list](http://groups.google.com/group/HMDB)
 - [SQLite FAQ](http://www.sqlite.org/faq.html)
 
 @warning Do not instantiate a single `HMDatabase` object and use it across multiple threads. Instead, use `<HMDatabaseQueue>`.
...
/** Create a `HMDatabase` object.
 
 An `HMDatabase` is created with a path to a SQLite database file.  This path can be one of these three:

 1. A file system path.  The file does not have to exist on disk.  If it does not exist, it is created for you.
 2. An empty string (`@""`).  An empty database is created at a temporary location.  This database is deleted with the `HMDatabase` connection is closed.
 3. `nil`.  An in-memory database is created.  This database will be destroyed with the `HMDatabase` connection is closed.

 For example, to create/open a database in your Mac OS X `tmp` folder:

    HMDatabase *db = [HMDatabase databaseWithPath:@"/tmp/tmp.db"];

 Or, in iOS, you might open a database in the app's `Documents` directory:

    NSString *docsPath = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES)[0];
    NSString *dbPath   = [docsPath stringByAppendingPathComponent:@"test.db"];
    HMDatabase *db     = [HMDatabase databaseWithPath:dbPath];
 ...
 @return `HMDatabase` object if successful; `nil` if failure.

 */

+ (instancetype)databaseWithPath:(NSString*)inPath;

/** Initialize a `HMDatabase` object.
...
/** Execute select statement
 ...
 Executing queries returns an `<HMResultSet>` object if successful, and `nil` upon failure.  Like executing updates, there is a variant that accepts an `NSError **` parameter.  Otherwise you should use the `<lastErrorMessage>` and `<lastErrorMessage>` methods to determine why a query failed.
 
 In order to iterate through the results of your query, you use a `while()` loop.  You also need to "step" (via `<[HMResultSet next]>`) from one record to the other.
 ...
 @return A `<HMResultSet>` for the result set upon success; `nil` upon failure. If failed, you can call `<lastError>`, `<lastErrorCode>`, or `<lastErrorMessage>` for diagnostic information regarding the failure.
 
 @see HMResultSet
 @see [`HMResultSet next`](<[HMResultSet next]>)
 @see [`sqlite3_bind`](http://sqlite.org/c3ref/bind_blob.html)
 */
...
- (HMResultSet *)executeQueryWithFormat:(NSString*)format, ... NS_FORMAT_FUNCTION(1,2);
...
- (HMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray *)arguments;
...
- (HMResultSet *)executeQuery:(NSString *)sql withParameterDictionary:(NSDictionary *)arguments;


// Documentation forthcoming.
- (HMResultSet *)executeQuery:(NSString*)sql withVAList: (va_list)args;
...
/** Adds SQL functions or aggregates or to redefine the behavior of existing SQL functions or aggregates.
 ...
 For example:
    ...
    [queue inDatabase:^(HMDatabase *adb) {
        ...
        int rowCount = 0;
        HMResultSet *ars = [adb executeQuery:@"select * from ftest where StringStartsWithH(foo)"];
        while ([ars next]) {
            rowCount++;
            DLog(@"Does %@ start with 'h'?", [rs stringForColumnIndex:0]);
        }
        HMDBQuickCheck(rowCount == 2);
    }];
...
/** Generate an `NSDateFormatter` that won't be broken by permutations of timezones or locales.
 ...
 Example:
    ...
    myDB.dateFormat = [HMDatabase storeableDateFormat:@"yyyy-MM-dd HH:mm:ss"];
 ...
 @warning Note that `NSDateFormatter` is not thread-safe, so the formatter generated by this method should be assigned to only one HMDB instance and should not be used for other purposes.
...
/** Set to a date formatter to use string dates with sqlite instead of the default UNIX timestamps.
 ...
 @param format Set to nil to use UNIX timestamps. Defaults to nil. Should be set using a formatter generated using HMDatabase::storeableDateFormat.
 ...
 @warning Note there is no direct getter for the `NSDateFormatter`, and you should not use the formatter you pass to HMDB for other purposes, as `NSDateFormatter` is not thread-safe.
 */
...
/** Objective-C wrapper for `sqlite3_stmt`
 ...
 This is a wrapper for a SQLite `sqlite3_stmt`. Generally when using HMDB you will not need to interact directly with `HMStatement`, but rather with `<HMDatabase>` and `<HMResultSet>` only.
...
@interface HMStatement : NSObject {
