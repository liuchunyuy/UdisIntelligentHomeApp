...
@class HMAsyncReadPacket;
@class HMAsyncWritePacket;
@class HMAsyncSocketPreBuffer;

extern NSString *const HMAsyncSocketException;
extern NSString *const HMAsyncSocketErrorDomain;

extern NSString *const HMAsyncSocketQueueName;
extern NSString *const HMAsyncSocketThreadName;

extern NSString *const HMAsyncSocketManuallyEvaluateTrust;
#if TARGET_OS_IPHONE
extern NSString *const HMAsyncSocketUseCFStreamForTLS;
#endif
#define HMAsyncSocketSSLPeerName     (NSString *)kCFStreamSSLPeerName
#define HMAsyncSocketSSLCertificates (NSString *)kCFStreamSSLCertificates
#define HMAsyncSocketSSLIsServer     (NSString *)kCFStreamSSLIsServer
extern NSString *const HMAsyncSocketSSLPeerID;
extern NSString *const HMAsyncSocketSSLProtocolVersionMin;
extern NSString *const HMAsyncSocketSSLProtocolVersionMax;
extern NSString *const HMAsyncSocketSSLSessionOptionFalseStart;
extern NSString *const HMAsyncSocketSSLSessionOptionSendOneByteRecord;
extern NSString *const HMAsyncSocketSSLCipherSuites;
#if !TARGET_OS_IPHONE
extern NSString *const HMAsyncSocketSSLDiffieHellmanParameters;
#endif

#define HMAsyncSocketLoggingContext 65535


enum HMAsyncSocketError
{
	HMAsyncSocketNoError = 0,           // Never used
	HMAsyncSocketBadConfigError,        // Invalid configuration
	HMAsyncSocketBadParamError,         // Invalid parameter was passed
	HMAsyncSocketConnectTimeoutError,   // A connect operation timed out
	HMAsyncSocketReadTimeoutError,      // A read operation timed out
	HMAsyncSocketWriteTimeoutError,     // A write operation timed out
	HMAsyncSocketReadMaxedOutError,     // Reached set maxLength without completing
	HMAsyncSocketClosedError,           // The remote peer closed the connection
	HMAsyncSocketOtherError,            // Description provided in userInfo
};
typedef enum HMAsyncSocketError HMAsyncSocketError;

////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
#pragma mark -
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

@interface HMAsyncSocket : NSObject

/**
 * HMAsyncSocket uses the standard delegate paradigm,
 ...
 * If you pass NULL, HMAsyncSocket will automatically create it's own socket queue.
 ...
 * 
 * For accepting incoming connections, this means HMAsyncSocket automatically supports both protocols,
 * and can simulataneously accept incoming connections on either protocol.
 * 
 * For outgoing connections, this means HMAsyncSocket can connect to remote hosts running either protocol.
 * If a DNS lookup returns only IPv4 results, HMAsyncSocket will automatically use IPv4.
 * If a DNS lookup returns only IPv6 results, HMAsyncSocket will automatically use IPv6.
 ...
 * When a connection is accepted, a new instance of HMAsyncSocket will be spawned to handle it,
 ...
 * 
 * Please note the recommended way of releasing a HMAsyncSocket instance (e.g. in a dealloc method)
 ...
 * it is treated similarly to a timeout - the socket is closed with a HMAsyncSocketReadMaxedOutError.
 ...
 * This is due to the fact that HMAsyncSocket does NOT copy the data. It simply retains it.
 ...
 * 
 * - HMAsyncSocketManuallyEvaluateTrust
 ...
 *     So HMAsyncSocket will invoke the delegate method socket:shouldTrustPeer: passing the SecTrustRef.
 *
 ...
 *
 * - HMAsyncSocketUseCFStreamForTLS (iOS only)
 *     The value must be of type NSNumber, encapsulating a BOOL value.
 *     By default HMAsyncSocket will use the SecureTransport layer to perform encryption.
 ...
 *     
 *     However, if you absolutely must, you can instruct HMAsyncSocket to use the old-fashioned encryption
 *     technique by going through the CFStream instead. So instead of using SecureTransport, HMAsyncSocket
 ...
 *     
 *     Thus all the other keys in the given dictionary will be ignored by HMAsyncSocket,
 ...
 *
 * - HMAsyncSocketSSLProtocolVersionMin
 * - HMAsyncSocketSSLProtocolVersionMax
 ...
 *
 * - HMAsyncSocketSSLDiffieHellmanParameters (Mac OS X only)
 *     The value must be of type NSData.
 ...
 *     You MUST use manual trust evaluation instead (see HMAsyncSocketManuallyEvaluateTrust).
 ...
 *     You MUST use HMAsyncSocketSSLProtocolVersionMin & HMAsyncSocketSSLProtocolVersionMin instead.
...
/**
 * HMAsyncSocket maintains thread safety by using an internal serial dispatch_queue.
 ...
 * - (dispatch_queue_t)newSocketQueueForConnectionFromAddress:(NSData *)address onSocket:(HMAsyncSocket *)sock
 * {
 ...
 *     return socketQueue;
 * }
 * - (void)socket:(HMAsyncSocket *)sock didAcceptNewSocket:(HMAsyncSocket *)newSocket
 * {
 ...
 * Example usage:
 * 
 * - (void)socket:(HMAsyncSocket *)sock didConnectToHost:(NSString *)host port:(uint16_t)port
 * {
...
@protocol HMAsyncSocketDelegate
@optional
...
**/
- (void)socket:(HMAsyncSocket *)sock didReadData:(NSData *)data withTag:(long)tag;
...
**/
- (void)socket:(HMAsyncSocket *)sock didReadPartialDataOfLength:(NSUInteger)partialLength tag:(long)tag;

/**
 * Called when a socket has completed writing the requested data. Not called if there is an error.
**/
- (void)socket:(HMAsyncSocket *)sock didWriteDataWithTag:(long)tag;

/**
 * Called when a socket has written some data, but has not yet completed the entire write.
 * It may be used to for things such as updating progress bars.
**/
- (void)socket:(HMAsyncSocket *)sock didWritePartialDataOfLength:(NSUInteger)partialLength tag:(long)tag;
...
**/
- (NSTimeInterval)socket:(HMAsyncSocket *)sock shouldTimeoutReadWithTag:(long)tag
...
**/
- (NSTimeInterval)socket:(HMAsyncSocket *)sock shouldTimeoutWriteWithTag:(long)tag
                                                                  elapsed:(NSTimeInterval)elapsed
                                                                bytesDone:(NSUInteger)length;
...
**/
- (void)socketDidCloseReadStream:(HMAsyncSocket *)sock;
 ...
 * 
 * Note: If the HMAsyncSocket instance is deallocated while it is still connected,
...
**/
- (void)socketDidDisconnect:(HMAsyncSocket *)sock withError:(NSError *)err;
...
**/
- (void)socket:(HMAsyncSocket *)sock didReceiveTrust:(SecTrustRef)trust
