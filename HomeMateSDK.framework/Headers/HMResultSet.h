...
/** Represents the results of executing a query on an `<HMDatabase>`.
...
@interface HMResultSet : NSObject {
    HMDatabase          *_parentDB;
    HMStatement         *_statement;
...
/** `HMStatement` used by result set. */

@property (atomic, retain) HMStatement *statement;

///------------------------------------
/// @name Creating and closing database
///------------------------------------

/** Create result set from `<HMStatement>`
 
 @param statement A `<HMStatement>` to be performed
 
 @param aDB A `<HMDatabase>` to be used
 
 @return A `HMResultSet` on success; `nil` on failure
 */

+ (instancetype)resultSetWithStatement:(HMStatement *)statement usingParentDatabase:(HMDatabase*)aDB;
