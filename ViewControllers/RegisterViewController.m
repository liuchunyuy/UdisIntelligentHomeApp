...
-(void)creatPhoneNumberTextField{
   ...
   // _phoneNumber.borderStyle = UITextBorderStyleRoundedRect;
    ...
    _phoneNumber.placeholder = @"请输入要注册的手机号";
...
-(void)goRegister2{
    ...
    sendCmd(cmd, ^(KReturnValue returnValue, NSDictionary *returnDic) {
        ...
        if (returnValue == KReturnValueSuccess) {
            NSLog(@"验证码发送成功");
            TestNumViewController *textNumVc = [[TestNumViewController alloc]init];
            [self presentViewController:textNumVc animated:YES completion:^{
                
            }];
        }else if(returnValue == KRegisterUserExist){
            [self popAlertWithMessage:@"该手机号已注册"];
        }else{
